...
\item Un alimentatore chiuso su due resistenze in parallello.
...
\plasduinodoc{un termometro stilizzato}

\plasduinosave

\labsubsection{Facoltativo: Stima della costante di decadimento del raffreddamento del termistore}
La costante di decadimento pu\`o essere stimata con una acquisizone lunga:
dopo aver scaldato il termistore in un foro vicino alla resistenza 
potete spostarlo in un foro a temperatura pi\`u bassa
e iniziare la presa dati finch\'e non vedete che la temperatura \`e stabile.
Ci si aspetta che l'andamento della temperatura col tempo sia un esponenziale decrescente
opportunamente scalato ($T_{\Delta}$) e traslato alla temperatura finale ($T_f$):
\begin{align}
  T(t; \alpha, T_f, T_{\Delta}) = T_{\Delta} e^{-\alpha t} + T_f
\end{align}
Tramite un fit dei minimi quadrati potete stimare il valore della costante di decadimento, $\alpha$.
Cosa succede se invece del raffreddamento studiamo il riscaldamento?
...
\input{sniptex/conducibilita_termica}
